...
final = read.csv(file.choose(), header = TRUE) #DUI_Beer_Data.csv file
...
# Load your data
breweries_file = read.csv(breweries_file, header = TRUE)
NROW(breweries_file)
...
ggplot(beer_brew_whole, aes(x = State, y = ABV, fill = IBU)) +
  geom_bar(stat = "identity") +
...
summary(beer_dat$ABV)

#Calculate mean value of ABV and IBU 
mean_ABV= mean(beer_dat$ABV)
mean_IBU = mean(beer_dat$IBU)

```

```{r abv_chart, echo=FALSE}
#Put a chart together showing the Alcohol by Volume in Beer
ggplot(beer_dat, aes(x = ABV * 100.0)) +
  ...
  geom_text(aes(x = mean_ABV, y = 200, label = "Mean ABV"), 
            vjust = 1, hjust = -0.1, size = 4, color = "red", label.position = "right") +
  labs(x = "Alcohol By Volume (%)", y = "Beer Count", title = "Alcohol by Volume \nin American Beers") +
  theme_wsj() +
  theme(plot.margin = unit(c(1, 1, 1, 1), "cm"),
        # plot.background = element_rect(fill = "white"),
        plot.title = element_text(hjust = 0.5),
...
ggplot(beer_brew_whole, aes(x = ABV * 100.0, y = IBU)) +
  ...
  labs(x = "Alcohol By Volume (%)", y = "International Bitterness Units", title = "IBU vs. ABV in \nAmerican Beers", subtitle = "Correlation = 0.652" ) +
  theme_wsj()   +
  theme(axis.title=element_text(size=12)) +
  theme(plot.margin = unit(c(1, 1, 1, 1), "cm"),
      # plot.background = element_rect(fill = "white"),
...
ggplot(bb_ales, aes(x = IBU, y = ABV * 100.0, color = IsIPA)) +
  ...
  labs(title = "IPA Prediction by \nABV and IBU", color = "", y = "Alcohol By Volume (%)", x="International Bitterness Units") +
  theme_wsj()   +
  theme(axis.title=element_text(size=12)) +
  theme(plot.margin = unit(c(1, 1, 1, 1), "cm"),
      # plot.background = element_rect(fill = "white"),
...
```{r corr_score}
#Run a correllation score ABV and DUI Arrest Rate
correlation <- cor(final$ABV, final$`DUI.Arrest.Rate..per.100K.`)
cat("The correlation between ABV and DUI Arrest Per 100,000 People is", round(correlation, 2))

```

```{r graph_findings, echo =FALSE}
# Create the variable that maps DUI Arrest to the color values
merged_data_frame <-merge(x = beer_brew_whole, y = final, by = "State")
merged_data_frame$color = cut(merged_data_frame$DUI.Arrest.Rate..per.100K., breaks = breaks, labels = colors[-1], include.lowest = TRUE)
...
ggplot(merged_data_frame, aes(x = geo_name, y = DUI.Arrest.Rate..per.100K., fill = color)) +
  geom_bar(stat = "identity", color = "black", alpha = 0.7) +
  abv_line +
  labs(x = "State", y = "DUI Arrest Rate per 100K / ABV") +
  ggtitle("DUI Arrest Rate \nper 100K by State") +
